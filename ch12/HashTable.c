...
int hash(char *key, int range) {
  ...
  for (i=0; i<strlen(key); i++) {
    BYTE value = (BYTE) key[i];//無正負的第i個key 
    hashCode += value;//hash+=|key[i]|
    hashCode %= range;//取餘數 
  }
  return hashCode;
}
...
int EntryCompare(Entry *e1, Entry *e2) {
  return strcmp(e1->key, e2->key);//一ㄍㄍ比 
}

HashTable* HashTableNew(int size) {
  Array *table = ArrayNew(size);//新建size的arr 
  int i;
...
void HashTableFree(HashTable *table) {
  ...
  for (ti=0; ti<table->size; ti++) {
    ...
    ArrayFree(hitArray, (FuncPtr1) EntryFree);//清光光 
  }
